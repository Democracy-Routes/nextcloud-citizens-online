...
@pytest.mark.parametrize(
    "relative_path",
    ["citizens_online/jobs/sweep.py", "citizens_online/jobs/runner.py", "citizens_online/jobs/handlers.py"],
)
...
def test_upload_path_reads_caption_config_before_touching_the_database():
    """settings.snapshot() must be read before the first query in upload_chunk,
    so its 30-second cache refresh cannot block writers."""
    import inspect

    from citizens_online.api import capture

    source = inspect.getsource(capture.upload_chunk)
    snapshot_at = source.index("settings_svc.snapshot()")
    session_at = source.index("with session_scope()")
    assert snapshot_at < session_at, (
        "settings.snapshot() must be called before a transaction opens — "
        "otherwise its OCS refresh holds the write lock"
    )


def test_upload_path_reads_the_body_before_opening_a_transaction():
    """The body read must not happen while the write lock is held: a slow
    uploader would otherwise block every other writer for the whole upload."""
    import inspect

    from citizens_online.api import capture

    source = inspect.getsource(capture.upload_chunk)
    body_at = source.index("await request.body()")
    session_at = source.index("with session_scope()")
    assert body_at < session_at, "read the request body before touching the database"


@pytest.mark.parametrize(
    "module_name, function_name",
    [
        ("citizens_online.services.analysis", "analyze_room"),
        ("citizens_online.services.analysis", "analyze_round"),
    ],
)
def test_handlers_commit_before_reading_provider_config(module_name, function_name):
    ...
    for marker in ("get_setting(", "_config(", "build_system_prompt("):
