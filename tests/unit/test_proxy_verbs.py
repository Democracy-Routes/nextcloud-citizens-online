...
to /apps/app_api/proxy/citizens_online/... is answered 405 by Nextcloud's own router:
...
def routes() -> list[tuple[str, str]]:
    ...
    assert len(found) > 35, f"route inventory looks wrong: {len(found)} routes"
...
def test_frontend_clients_never_send_an_unproxyable_verb():
    for client in ("frontend/src/api.ts",):
...
def test_every_download_refuses_to_be_cached(routes):
    ...
    from citizens_online.api import reports

    downloads = {
        "report.md": (reports, "report_markdown"),
        "report.pdf": (reports, "report_pdf"),
        "report.json": (reports, "report_json"),
    }
    missing = []
...
def test_the_ui_bundle_is_revalidated_not_cached_for_an_hour():
    ...
    from citizens_online import main
