...
def configure_database(db_url: str) -> Engine:
    ...
    _read_session_factory = sessionmaker(
        bind=_engine.execution_options(co_read_only=True), expire_on_commit=False
    )
    return _engine
...
def _begin_immediate(conn) -> None:
    ...
    if conn.get_execution_options().get("co_read_only"):
        return
...
def read_only_scope() -> Iterator[Session]:
    ...
    Bound to an engine carrying the `co_read_only` execution option,
