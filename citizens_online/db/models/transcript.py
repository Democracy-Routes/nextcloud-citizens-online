...
class TranscriptSegment(Base):
    ...
    # Online, the speaker is known from the stream, not guessed acoustically:
    # the label is the participant's display name and the link is a real FK.
    speaker_label: Mapped[str] = mapped_column(String(120), default="")
    participant_id: Mapped[str | None] = mapped_column(
        ForeignKey("participants.id", ondelete="SET NULL")
    )
    nc_user_id: Mapped[str] = mapped_column(String(64), default="")
    # live | postcall | chat — where this line came from
