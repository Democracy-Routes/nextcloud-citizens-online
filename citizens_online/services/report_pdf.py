...
"""PDF rendering of the session report (fpdf2).
...
from citizens_online.services.report import TYPE_LABELS_SINGULAR, group_findings_by_type
...
def _finding(pdf: _ReportPDF, finding: dict, cross: bool) -> None:
    ...
    if cross and finding["mentioned_room_count"]:
        pdf.text_block(
            f"Mentioned at {finding['mentioned_room_count']} room(s)",
...
def render_pdf(report: dict, logo_path: Path | None = None,
    ...
    footer_text = " · ".join(part for part in (organization_name, session_data["name"]) if part)
    pdf = _ReportPDF(footer_text or session_data["name"])
    pdf.add_page()
    ...
    pdf.multi_cell(title_w, 9, session_data["name"], new_x="LMARGIN", new_y="NEXT")
    ...
    if report.get("is_final"):
        ...
        pdf.text_block(
            "INTERIM REPORT · findings are still being reviewed",
            size=9, style="B", color=AMBER, height=4.6,
        )
    generated = datetime.now(UTC).strftime("%-d %B %Y")
    pdf.text_block(
        f"{generated} · {session_data['participants']} participants · "
        f"{session_data['rooms']} rooms · {session_data['language'].upper()}",
        size=9.5, color=MUTED,
    )
    pdf.ln(2)
    if session_data["description"]:
        pdf.text_block(session_data["description"])
        pdf.ln(2)
    pdf.eyebrow("Method")
    ...
    if summaries:
        ...
        pdf.text_block(
            "AI-generated overview of each round across all rooms; details and "
    ...
    for round_ in report["rounds"]:
        ...
        if round_["question"]:
            ...
            pdf.ln(1.5)
        if round_["summary"]:
            pdf.eyebrow("AI summary — all rooms")
            pdf.text_block(round_["summary"], height=5)
            pdf.ln(1.5)

        for type_, label, group in group_findings_by_type(round_["cross_room"]):
        ...
        if (
            not round_["summary"]
            and not round_["cross_room"]
            and not rooms_with_content
        ):
