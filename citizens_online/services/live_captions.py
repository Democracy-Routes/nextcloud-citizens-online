...
from citizens_online.core.speaking.meter import SpeechMeter
from citizens_online.logging_setup import get_logger
from citizens_online.services import live_audio
...
class _BaseSession:
    ...
    def __init__(
        ...
        # only needed to file the persisted transcript under its session, so
        # deleting the session takes it too
        self.session_id = session_id
        # Online there is one microphone per person, so the speaker of every
        # line is known rather than guessed: no diarization, no SPEAKER_01.
        self.speaker_name = speaker_name
        # speaking time is measured from this same audio (spec §12)
    ...
    def add_line(
        ...
        if len(self.lines) >= MAX_TRANSCRIPT_LINES:
            ...
            return
        line = {"t": start, "text": text, "speaker": speaker or self.speaker_name or None}
        if end is not None:
            line["end"] = end
        if words:
            line["words"] = words
...
def _whisper_window_request(
    ...
    from citizens_online.providers.transcription import whisper as whisper_provider
...
class LiveCaptionManager:
    ...
    def _resolve_vosk_model(config: dict, language: str) -> str:
        ...
        from citizens_online.services.provider_config import vosk_model_path
    ...
    def feed(
        self, recording_id: str, data: bytes, config: dict, language: str, session_id: str = ""
    ) -> None:
        ...
        try:
            ...
            asyncio.run_coroutine_threadsafe(
                self._feed_async(recording_id, data, config, language, session_id), self._loop
            )
        except Exception:
    ...
    def speaking(self, recording_id: str) -> dict | None:
        """Live speaking-time snapshot for one participant's stream."""
        session = self._sessions.get(recording_id)
        return session.meter.snapshot() if session else None
    ...
    async def _pump_pcm(self, session: "_BaseSession", stream) -> None:
        ...
        try:
            while True:
                ...
                if pcm is None:
                    break
                # Speaking time is measured here, on the audio itself, before
                # the transcriber sees it — so it keeps working even when
                # transcription is switched off or the engine is failing
                # (spec §12: never from transcript word counts).
                session.meter.feed(pcm)
    ...
    async def _persist_transcript(self, session: "_BaseSession") -> None:
        ...
        path = live_caption_path(
            get_settings().app_persistent_storage, session.session_id, session.recording_id
        )
