...
async def run_forever(stop_event: asyncio.Event) -> None:
    ...
    while not stop_event.is_set():
        ...
        now = time.monotonic()
        # The deliberation clock must run even while jobs are queued: it is
        # what ends a round on time and what makes the facilitator speak.
        last_tick = await asyncio.to_thread(tick_if_due, last_tick)
