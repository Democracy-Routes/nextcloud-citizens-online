...
def db_path(root: Path) -> Path:
    return root / "citizens_online.db"
...
def recording_dir(root: Path, session_id: str, round_id: str, room_id: str, recording_id: str) -> Path:
    ...
    return recordings_dir(root) / session_id / round_id / room_id / recording_id
...
def assembled_dir(root: Path, session_id: str) -> Path:
    return root / "assembled" / session_id
...
def exports_dir(root: Path, session_id: str) -> Path:
    """Generated ZIP archives (audio bundle, portable session export)."""
    return root / "exports" / session_id


def device_log_path(root: Path, client_session_id: str) -> Path:
    """Per-recorder-session shipped client logs (JSONL). session_id is a
    server-generated UUID, never client input."""
    return root / "logs" / "devices" / f"{client_session_id}.jsonl"


def live_caption_path(root: Path, session_id: str, recording_id: str) -> Path:
    ...
    once became.
    """
    return root / "live_captions" / session_id / f"{recording_id}.json"


def purge_session_storage(
    root: Path, session_id: str, recorder_session_ids: Sequence[str] = ()
) -> None:
    ...
    shipped diagnostic logs). session_id is a server-generated UUID, never
    client input.
    ...
    for subdir in ("recordings", "assembled", "transcripts", "exports", "live_captions"):
        shutil.rmtree(root / subdir / session_id, ignore_errors=True)
